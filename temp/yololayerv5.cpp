...
namespace nvinfer1
{
    ...
    {
         std::cout<<"========== yololayerV5 constructor without thread read =========="<<std::endl;
        ...
        std::cout<<"Kernel count >> "<<mKernelCount<<std::endl;
    ...
    {
        std::cout<<"========== yololayerV5 constructor with thread read =========="<<std::endl;
        using namespace Tn;
    ...
    float LogistV5(float data) { return 1.0f / (1.0f + expf(-data)); }

    void CalDetection(const float *input, float *output, int noElements, const int netwidth, const int netheight, int maxoutobject, int yoloWidth, int yoloHeight, const float anchors[CHECK_COUNT * 2], int classes, int outputElem)
    {
    ...
    void YoloLayerPluginV5::forwardGpu(const float *const * inputs, float* output, cudaStream_t stream, int batchSize) noexcept
    {
        ...
        {
            std::cout<<"forwardGPU -> Kernel loop"<<std::endl;
            ...
            // CalDetection(inputs[i], output, numElem, mYoloV5NetWidth, mYoloV5NetHeight, mMaxOutObject, yolo.width, yolo.height, (float *)mAnchor[i], mClassCount, outputElem);
            CalDetection(inputs[i], output, numElem, mYoloV5NetWidth, mYoloV5NetHeight, mMaxOutObject, yolo.width, yolo.height, (float *)mAnchor[i], mClassCount, outputElem);
        }
    }
    ...
    {
         std::cout<<"========== plugin creator =========="<<std::endl;
