...
cv::Rect get_rect(cv::Mat& img, float bbox[4], int w, int h, int cls) {
    ...
    if (r_h > r_w) {
        l = static_cast<int>(bbox[0] - bbox[2] / 2.f);
        r = static_cast<int>(bbox[0] + bbox[2] / 2.f);
        t = static_cast<int>(bbox[1] - bbox[3] / 2.f - (h - r_w * img.rows) / 2);
        b = static_cast<int>(bbox[1] + bbox[3] / 2.f - (h - r_w * img.rows) / 2);
        l = static_cast<int>(l / r_w);
        r = static_cast<int>(r / r_w);
        t = static_cast<int>(t / r_w);
        b = static_cast<int>(b / r_w);
    } else {
        l = static_cast<int>(bbox[0] - bbox[2] / 2.f - (w- r_h * img.cols) / 2);
        r = static_cast<int>(bbox[0] + bbox[2] / 2.f - (w - r_h * img.cols) / 2);
        t = static_cast<int>(bbox[1] - bbox[3] / 2.f);
        b = static_cast<int>(bbox[1] + bbox[3] / 2.f);
        l = static_cast<int>(l / r_h);
        r = static_cast<int>(r / r_h);
        t = static_cast<int>(t / r_h);
        b = static_cast<int>(b / r_h);
    }
...
std::map<std::string, Weights> loadWeights_yolo(const std::string file) {
...
IScaleLayer* addBatchNorm2dDet(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, std::string lname, float eps) {
...
ILayer* convBlock(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int outch, int ksize, int s, int g, std::string lname) {
    ...
    conv1->setNbGroups(g);
    IScaleLayer* bn1 = addBatchNorm2dDet(network, weightMap, *conv1->getOutput(0), lname + ".bn", 1e-3f);
...
ILayer* bottleneckCSP(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int c1, int c2, int n, bool shortcut, int g, float e, std::string lname) {
    ...
    IScaleLayer* bn = addBatchNorm2dDet(network, weightMap, *cat->getOutput(0), lname + ".bn", 1e-4f);
...
ILayer* compressorblock(INetworkDefinition* network, std::map<std::string, Weights>& weightMap, ITensor& input, int inch, int outch, int ksize, std::string lname, int w, int h, int cls) {
    ...
    std::cout << "======== 3" << pool1->getOutput(0)->getDimensions().d[0] << " " << pool1->getOutput(0)->getDimensions().d[1] << " " << pool1->getOutput(0)->getDimensions().d[2] << "\n";
